...
export function ClusterProvider({ children }) {
  ...
  // Poll FastAPI Server /api/status with local merge guard
  useEffect(() => {
    const fetchStatus = async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (data.nodes) {
            setNodes(prevNodes => {
              ...
              const customNodes = prevNodes.filter(n => !serverNodeIds.has(n.id.toLowerCase()));
              return [...data.nodes, ...customNodes];
            });
          }
        ...
        // Fallback to local state if FastAPI is connecting
      }
    };
  ...
  // Node Management (Optimistic local update + Backend sync)
  const addNode = async (newNode) => {
    setNodes(prev => {
      if (prev.some(n => n.id.toLowerCase() === newNode.id.toLowerCase())) {
        return prev.map(n => n.id.toLowerCase() === newNode.id.toLowerCase() ? newNode : n);
      }
      return [...prev, newNode];
    });
    ...
    try {
      ...
      console.warn('Backend offline, node saved in local React state');
    }
