...
export function ConnectNodeModal() {
  ...
  const endpoint = `${window.location.origin}${window.location.pathname.replace(/[^/]*$/, '')}api.php?action=ingest`;
  const basePath = window.location.pathname.replace(/[^/]*$/, '');
  const host     = lanIp || 'YOUR-LAN-IP';
  const url      = `http://${host}:${lanPort}${basePath}api.php?action=ingest`;
  const agentUrl = `http://${host}:${lanPort}${basePath}agents/windows-agent.ps1`;
  const instUrl  = `http://${host}:${lanPort}${basePath}agents/install-windows-agent.ps1`;
